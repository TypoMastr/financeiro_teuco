  ...
  return (
    <>
      ...
      <motion.nav
        ...
        className="lg:hidden fixed bottom-4 left-4 right-4 h-20 bg-card/80 dark:bg-dark-card/80 backdrop-blur-xl border border-border dark:border-dark-border z-50 no-print rounded-2xl shadow-xl"
      >
        <div className="grid grid-cols-2 items-stretch h-full max-w-lg mx-auto">
          {Object.entries(mainNavItems).map(([key, menu]) => (
            <div key={key} className="relative flex flex-col items-center justify-center">
              ...
              <motion.button
                onClick={() => handleMainMenuClick(key as ActiveMenu)}
              ...
              >
                <div className="relative w-7 h-7">
                  <AnimatePresence initial={false}>
                    ...
                    >
                      {activeMenu === key ? <X className="h-7 w-7 text-primary" /> : <menu.icon className="h-6 w-6" />}
                    </motion.div>
                  </AnimatePresence>
                </div>
                <span className={`text-xs font-bold transition-colors ${activeMenu === key ? 'text-primary' : ''}`}>{menu.label}</span>
              </motion.button>
